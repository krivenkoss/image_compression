"""Study 01 — noisy vs compressed.

Replicates the legacy ``Main OOP cycle (compare with noised image)`` block
from oop.py.  Edit the matching INI for parameter changes — do not hard-code
values here.
...
# Flat import from project root (run as: python -m studies.study_01_...)
...
def main() -> None:
    ...
    reference = cfg["study"]["reference"]        # "noisy" | "original"
        ...
        for image_name in cfg["images"]["filenames"]:
            ...
            for noise_kind in cfg["noise"]["kinds"]:
                for std in cfg["noise"]["std_values"]:
                    ...
                    ref = noised if reference == "noisy" else image
                    for p in tqdm(
                        cr_range,
    ...
    if not rows:
        print("No results collected — check that images exist in images/ and coders are available.")
        return
